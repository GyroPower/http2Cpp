...
- Python 3.14
- Clang or Gcc (Linux)
- MSVC/Clang Visual Studio, Gcc from ucrt64 msys2 or Clang from Clang64 msys2 (Windows)
...
- CMake 4.2 (minimum) 
- Ninja or Make for build (Useful for non Visual Studio Windows environment)
...
The build-install-cmake-system uses the libraries build and installed in the prefix path, use `-h` to get info of the arguments, to avoid run on sudo or as admin, is recommended to have defined the CMAKE_PREFIX_PATH env variable
```bash
...
This section explains that those python scripts with "develop" is for just that, generates build files but does not build and install the library, it includes the example directory, just for toying or local development.
...
`build-cmake-system-develop.py` is the same as above

test_server.py is for bench mark, need h2load in the path, it's from nghttp2 library, run it and modify it if you need for select port and url request, the program cannot be build from the conan package by default with openssl4, you need to have the work around to get it, maybe build the package apart, locate and add h2load binary to the path 
